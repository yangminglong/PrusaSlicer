...
bool PrusaLink::test_with_method_check(wxString& msg, bool& use_put ) const
{
    ...
    .on_complete([&, this](std::string body, unsigned) {
        ...
        try {
            ...
            // remove when method use is defined by additional information (code bellow but will reqiure correct var names)
            use_put = version_check(text);
#if 0
            const auto features_ptree = ptree.get_optional<pt::ptree>("features");
            if (!features_ptree)
                return;
            const auto put_upload = features_ptree.get().get_optional<bool>("put-upload");
            if (!put_upload)
                return;
            use_put = *put_upload;
#endif
...
bool PrusaLink::upload(PrintHostUpload upload_data, ProgressFn prorgess_fn, ErrorFn error_fn) const
{
    ...
    std::string storage_path = (use_put? "api/v1/local" : "api/files/local");
    ...
    BOOST_LOG_TRIVIAL(error) << boost::format("%1%: Uploading file %2% at %3%, filename: %4%, path: %5%, print: %6%, method: %7%")
        % name
        % upload_data.source_path
        % url
        % upload_filename.string()
...
bool PrusaLink::put_inner(PrintHostUpload upload_data, std::string url, const std::string& name, ProgressFn prorgess_fn, ErrorFn error_fn) const
{
    ...
    http.set_put_body(upload_data.source_path)
        .header("print", upload_data.post_action == PrintHostPostUploadAction::StartPrint ? "true" : "false")
        .header("Content-Type", "text/x.gcode")
        //.header("path", upload_parent_path.string())
        .on_complete([&](std::string body, unsigned status) {
            BOOST_LOG_TRIVIAL(debug) << boost::format("%1%: File uploaded: HTTP %2%: %3%") % name % status % body;
        })
        .on_error([&](std::string body, std::string error, unsigned status) {
