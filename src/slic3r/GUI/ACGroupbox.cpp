...
WXDWORD ACGroupBox::MSWGetStyle(long style, WXDWORD *exstyle) const
{
...
void ACGroupBox::GetBordersForSizer(int *borderTop, int *borderOther) const
{
...
bool ACGroupBox::SetBackgroundColour(const wxColour& colour)
{
...
void ACGroupBox::PaintBackground(wxDC& dc, const RECT& rc)
{
...
ACGroupBoxSizer::ACGroupBoxSizer(int orient, wxWindow *win, const wxString& s)
                : wxBoxSizer(orient),
                  m_groupBox(new ACGroupBox(win, wxID_ANY, s))
{
    // same as above
    m_groupBox->SetContainingSizer(this);
}
...
ACGroupBoxSizer::~ACGroupBoxSizer()
{
    ...
    if ( m_groupBox )
        m_groupBox->WXDestroyWithoutChildren();
}

void ACGroupBoxSizer::RepositionChildren(const wxSize& minSize)
{
    int top_border, other_border;
    m_groupBox->GetBordersForSizer(&top_border, &other_border);
    ...
    if (m_groupBox->GetChildren().GetCount() > 0)
    {
#if defined( __WXGTK20__ )
        // if the ACGroupBox has created a wxPizza to contain its children
        // (see ACGroupBox::AddChild) then we need to place the items it contains
...
#else
        // if the ACGroupBox has children, then these windows must be placed
...
wxSize ACGroupBoxSizer::CalcMin()
{
    ...
    const int boxWidth = m_groupBox->GetBestSize().x;
    if ( ret.x < boxWidth )
        ret.x = boxWidth;
