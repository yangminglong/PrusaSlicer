...
ACIMSlider::ACIMSlider(int lowerValue, int higherValue, int minValue, int maxValue, long style)
{
...
int ACIMSlider::GetActiveValue() const
{
...
void ACIMSlider::SetLowerValue(const int lower_val)
{
    m_selection   = ssLower;
...
void ACIMSlider::SetHigherValue(const int higher_val)
{
...
void ACIMSlider::SetSelectionSpan(const int lower_val, const int higher_val)
{
...
void ACIMSlider::SetMaxValue(const int max_value)
{
    m_max_value = max_value;
    set_as_dirty();
}

void ACIMSlider::SetSliderValues(const std::vector<double> &values)
{
    m_values = values;
}

Info ACIMSlider::GetTicksValues() const
{
...
void ACIMSlider::SetTicksValues(const Info &custom_gcode_per_print_z)
{
...
void ACIMSlider::SetLayersTimes(const std::vector<float> &layers_times, float total_time)
{
...
void ACIMSlider::SetLayersTimes(const std::vector<double> &layers_times)
{
...
void ACIMSlider::SetDrawMode(bool is_sequential_print)
{
    m_draw_mode = is_sequential_print   ? dmSequentialFffPrint  : 
                                          dmRegular; 
}

void ACIMSlider::SetModeAndOnlyExtruder(const bool is_one_extruder_printed_model, const int only_extruder, bool can_change_color)
{
...
void ACIMSlider::SetExtruderColors( const std::vector<std::string>& extruder_colors)
{
    m_extruder_colors = extruder_colors;
}

bool ACIMSlider::IsNewPrint()
{
...
void ACIMSlider::post_ticks_changed_event(Type type)
{
    m_tick_change_event_type = type;
    m_is_need_post_tick_changed_event = true;
}

void ACIMSlider::add_custom_gcode(std::string custom_gcode)
{
...
void ACIMSlider::add_code_as_tick(Type type, int selected_extruder)
{
    if (m_selection == ssUndef) return;
    const int tick = m_selection == ssLower ? m_lower_value : m_higher_value;
...
bool ACIMSlider::check_ticks_changed_event(Type type)
{
...
bool ACIMSlider::switch_one_layer_mode()
{
...
void ACIMSlider::draw_background(const ImRect& groove) {
...
bool ACIMSlider::horizontal_slider(const char* str_id, int* value, int v_min, int v_max, const ImVec2& pos, const ImVec2& size, float scale)
{
...
void ACIMSlider::draw_colored_band(const ImRect& groove, const ImRect& slideable_region) {
    if (m_ticks.empty())
        return;
...
void ACIMSlider::draw_ticks(const ImRect& slideable_region) {
...
bool ACIMSlider::vertical_slider(const char* str_id, int* higher_value, int* lower_value, std::string& higher_label, std::string& lower_label,int v_min, int v_max, const ImVec2& pos,const ImVec2& size, SelectedSlider& selection, bool one_layer_flag, float scale)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ACIMSlider::render(int canvas_width, int canvas_height)
{
    bool result = false;
...
void ACIMSlider::render_input_custom_gcode()
{
    if (!m_show_custom_gcode_window)
        return;
    ImGuiWrapper& imgui = *wxGetApp().imgui();
...
void ACIMSlider::set_scale(float scale)
{
    if(m_scale != scale) m_scale = scale;
}

void ACIMSlider::correct_lower_value()
{
...
void ACIMSlider::correct_higher_value()
{
...
bool ACIMSlider::is_wipe_tower_layer(int tick) const
{
...
std::string ACIMSlider::get_label(int tick, LabelType label_type)
{
...
double ACIMSlider::get_double_value(const SelectedSlider &selection)
{
...
int ACIMSlider::get_tick_from_value(double value, bool force_lower_bound /* = false*/)
{
...
float ACIMSlider::get_pos_from_value(int v_min, int v_max, int value, const ImRect& rect) {
...
std::string ACIMSlider::get_color_for_tool_change_tick(std::set<TickCode>::const_iterator it) const
{
...
std::array<int, 2> ACIMSlider::get_active_extruders_for_tick(int tick) const
{
