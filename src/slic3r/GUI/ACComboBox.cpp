    ...
    : drop(texts, icons)
{
    ...
    ACTextInput::Create(parent, "", value, (style & CB_NO_DROP_ICON) ? "" : "drop_down", pos, size,
                      style | wxTE_PROCESS_ENTER);

    ACStateColor selectorBg(std::make_pair(AC_COLOR_DROPDOWN_ITEM_BG_NOR, (int) ACStateColor::Normal)
    ...
    drop.SetCornerRadius(FromDIP(6));
    //drop.SetBorderColor(std::make_pair(*wxWHITE, (int) ACStateColor::Normal));
    drop.SetBackgroundColour(AC_COLOR_WHITE);
    
    drop.SetSelectorBackgroundColor(selectorBg);
    //drop.SetSelectorBorderColor(selectorBg);
    drop.Create(this, style & DD_STYLE_MASK);

    if (style & wxCB_READONLY) {
        GetTextCtrl()->Hide();
        ACTextInput::SetCornerRadius(FromDIP(6));
        ACTextInput::SetFont(ACLabel::Body_14);
...
int ACComboBox::Append(const wxString &item, const wxBitmap &bitmap)
{
...
wxBitmap ACComboBox::GetItemBitmap(unsigned int n) { return icons[n]; }
